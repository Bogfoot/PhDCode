...
# in order to switch between Widows 10 and Linux just comment in/out the appoprite first few lines 
# in the initialize() function. the difference is in how OS finds the camera

# right now, this definetly works for ONE camera on Windows 10. I have not tested it for more than one, maybe
# that requires a few extra lines. Linux should work without a problem
...
class Camera:
  ...
  def initialize(self):
    ...
    # Linux ####################
    # videostr = "/dev/video%d" % (self.cam_num) #Linux
    # self.cap = cv2.VideoCapture(videostr) #Linux
    # print("video string: ",videostr) #Linux
    ############################
...
#-------------- added by Kristof on 29.3.2021
    ...
    xmax = np.where(frame[:,:,2]==frame[:,:,2].max())[0][0]
    ymax = np.where(frame[:,:,2]==frame[:,:,2].max())[1][0]
    
    closeup = frame[xmax-boxsize//2:xmax+boxsize//2,ymax-boxsize//2:ymax+boxsize//2,2]

    xstd = frame[:, ymax-averagingsize//2:ymax+averagingsize//2, 2].mean(axis=1).std()
    ystd = frame[xmax-averagingsize//2:xmax+averagingsize//2, :, 2].mean(axis=0).std()

    fig, ax = plt.subplots(3)
    ax[0].imshow(frame[:,:,2])
    ax[1].imshow(closeup)
    # ax[2].imshow(np.ones((100,100)))
    ax[2].text(x=.50, y=.8, s=r"$\sigma_x = {:.2f}$".format(xstd), fontsize=fontsize)
    ax[2].text(x=.50, y=.20, s=r"$\sigma_y = {:.2f}$".format(ystd), fontsize=fontsize)
