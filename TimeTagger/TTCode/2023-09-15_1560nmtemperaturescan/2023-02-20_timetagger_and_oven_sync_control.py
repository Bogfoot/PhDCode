...
#dodaj še string iz dveh delov - pot do onedriva in pot znotraj onedriva
# if socket.gethostname()=='FMF1287': #lab desktop
#     basedir = "C:\\Users\\Admin\\OneDrive - Univerza v Ljubljani"
#     onedrive_path = '\\Lab_folder\\02_Data_2\\2023-03-01_timetagger&oven' #probably different

if socket.gethostname()=='FMF1287': #lab deskto
    basedir = 'C:\\Users\\Admin\\Desktop'
    onedrive_path = '\\2023-09-06_ data_2\\2023-03-01_timetagger&oven' #probably different

elif socket.gethostname()=='DESKTOP-RS0QF32': #ziga's FMF laptop
    basedir = 'C:\\Users\\ziga'
    onedrive_path = '\\OneDrive - Univerza v Ljubljani\\Kaltenlab\\2023-03-01_timetagger&oven'

elif socket.gethostname()== 'Zigas-MacBook-Pro.local':  #ziga's home laptop
    basedir = "/Users/Ziga/Documents/OneDrive - Univerza v Ljubljani/"
    onedrive_path = "Kaltenlab/2023-02-20_timetagger&oven"
else:
    raise("Computer not in the database, so current directory not set")


directorypath = basedir + onedrive_path
# os.chdir(directorypath)
os.chdir(r"C:\Users\Admin\OneDrive\Desktop\2023-09-15_1560nmtemperaturescan") #change
###########################
...
print("If everything goes according to plan, the scan will take approx. ",(sleepy_sleepy_timetagger * n + (2 * n * sleepy_sleepy_oven)) // (60*60)," h")
#
# Initialize the quTAG device
tt = qt.QuTAG()
timebase = tt.getTimebase()
...
n = int((temperature_end - temperature_start)/temperature_step) + 1 # +1 because of the inital temperature
temperature = np.linspace(temperature_start,temperature_end,n)
...
data_file_name =  str(date.today()) + "_SPDC_1560_phase_matching_fine_tsweep_" + str(temperature_start) + "-" + str(temperature_end) +  "degC.txt"
...
f.write("# This measurement DOES NOT inculde the coincidence stage where we seperate single photons based on polarization per channel. \n")
...
f.write("# A SPD is connected to one branch of the coincidence stage, while the other SPD is connected directly to the V photon branch on the temp scan\n")
...
for i in range(n):
    ...
    data, updates = tt.getCoincCounters()
    f = open(file_name, "a")
    f.write(str(round(temperature[i],2)) + "    " + str(data[1]) + "    " + str(data[2]) + "    " + str(data[5]) + "\n")
    f.close()
...
plt.ylabel("Counts []")
plt.title("Phase matching measurment, pump polarization: H, 1560 nm, coinc_window = 1 ns, T_int = 60 s, alligned at 37,39 °C")
plt.legend()
# plt.plot(data2[0],data2[1])
# plt.plot(temperature,coincidences_measurment)
...
plt.show()
# Save figure if
plot_filename = str(os.getcwd()) + "\\Pictures\\" + "tempscan-SPDC-" + str(date.today()) + ".png"
...
# plt.plot(data1[0], data1[1])
# plt.plot(data1[0], data1[2])
plt.plot(data1[0], (data1[1] + data1[2])/10000, label = "H pump, H + V polarization [xe4]")
plt.plot(data1[0], (data2[1] + data2[2])/10000, label = "V pump, H + V polarization [xe4]")
...
plt.title("Phase matching measurment, 1550 nm,coinc_window = 1 ns ,T_int = 60 s, alligned at 22 C")
plt.legend()
# plt.plot(data2[0],data2[1])
# plt.plot(temperature,coincidences_measurment)
...
plt.title("Coincidence measurment, T_int = 60 s, coinc_window = 10 ns, alligned at 22 C")
# plt.plot(data2[0],data2[1])
# plt.plot(temperature,coincidences_measurment)
plt.legend()
plt.show()
...
f.write("# full measurement of the phasematching condition, now for controler 2 and oven 2 \n")
...
f.write("# periodic pooling: 6.57 um (same as the first measurement for the first controler")
...
f.write("# this measurement now includes the coincidence stage where we seperate single photons based on polarization per channel \n")
f.write("# i am now repeating this measurment to see if the measurment is reproducable or the controler introduces an offset \n")
f.write("# --------------------------------- \n")
