...
#2022-07-12 qutag_histogram.dll has to be in the same folder as the other dlls
...
class QuTAG:
    ...
    def __init__(self):
        ...
        if (platform.system()=="Windows"):
            ...
            #path2= "C:\\Users\\ziga\\Desktop\\test za C\\"
            self.coincLib = ctypes.windll.LoadLibrary('qutag_histogram.dll')

        if (platform.system()=="Linux"):#if you want to swith to linux you also need to change self.tdclib into self.qutools_dll throughout this code... or just use the other code
            self.tdclib = ctypes.cdll.LoadLibrary('libtdcbase.so')
            self.coincLib = ctypes.cdll.LoadLibrary('/home/rainer/OneDrive/Lab_folder/Programs/python/timetagger/qutag_histogram.so')
    ...
    def declareAPI(self):
        ...
        # self.qutools_dll.TDC_determineCoincidenceHistogram.argtypes =[ctypes.data_as(ctypes.POINTER(ctypes.c_int64)), ctypes.data_as(ctypes.POINTER(ctypes.c_int8)), ctypes.c_int64, ctypes.c_int64, ctypes.c_int64, ctypes.c_int64, ctypes.c_double, ctypes.byref, ctypes.byref, ctypes.data_as(ctypes.POINTER(ctypes.int64)), ctypes.int64]
        # self.qutools_dll.TDC_determineCoincidenceHistogram = ctypes.c_int32


        self.coincLib.determineCoincidencesStartBlock.argtypes = [ctypes.POINTER(ctypes.c_int64), ctypes.POINTER(ctypes.c_int8), ctypes.c_int32, ctypes.c_int32, ctypes.c_int32, ctypes.c_int32, ctypes.c_double, ctypes.c_double]
        self.coincLib.determineCoincidencesStartBlock.restype = ctypes.c_int32

        self.coincLib.determineCoincidenceBlock.argtypes = [ctypes.POINTER(ctypes.c_int64), ctypes.POINTER(ctypes.c_int8), ctypes.c_int32, ctypes.c_int32, ctypes.c_int32, ctypes.c_int32, ctypes.c_int32, ctypes.c_double, ctypes.c_double]
        self.coincLib.determineCoincidenceBlock.restype = ctypes.c_int32

        self.coincLib.determineCoincidences.argtypes = [ctypes.POINTER(ctypes.c_int64), ctypes.POINTER(ctypes.c_int8), ctypes.c_int32, ctypes.c_int32, ctypes.c_int32, ctypes.c_int32, ctypes.c_double, ctypes.c_double]
        self.coincLib.determineCoincidences.restype = ctypes.c_int32
        ...
        self.coincLib.determineCoincidenceHistogram.argtypes = [ctypes.POINTER(ctypes.c_longlong), ctypes.POINTER(ctypes.c_int8), ctypes.c_longlong, ctypes.c_longlong, ctypes.c_longlong, ctypes.c_longlong, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.POINTER(ctypes.c_longlong), ctypes.c_longlong]
        self.coincLib.determineCoincidenceHistogram.restype = ctypes.c_longlong
...
# we get the set of last coincidences. "dt" is the coincidence window
# histlen is the number of entries in the histogram
# the function returns a tupe of the form (hist, T1, T2),
# where "hist" is a numpy array containing the histogram for time delays
# between "T1" and "T2"
# "n" is the block size used for generating the histogram block by block from
# the timetags supplied
#
# (rainer)
    def getCoincidenceHistogram(self, tags, chs, valid, ch1, ch2, n, dt, T1, T2, histlen):
        hist = np.zeros(int(histlen), dtype=np.int64)
        self.coincLib.determineCoincidenceHistogram(tags.ctypes.data_as(ctypes.POINTER(ctypes.c_longlong)), chs.ctypes.data_as(ctypes.POINTER(ctypes.c_int8)), ctypes.c_longlong(valid), ctypes.c_longlong(ch1), ctypes.c_longlong(ch2), ctypes.c_longlong(n), ctypes.c_double(dt),ctypes.c_double(T1),ctypes.c_double(T2),hist.ctypes.data_as(ctypes.POINTER(ctypes.c_longlong)),ctypes.c_longlong(histlen))
        return hist
