...
Int64 determineCoincidenceHistogram180722(Int64* tags, Int8* channels, Int64 valid, Int64 ch1, Int64 ch2, Int64 n, double dt, double* T1out, double* T2out, Int64* hist, Int64 histlen)
{
	...
	Int64 T1int,
	      T2int,
	      Tmin1, Tmin2, Tmax1, Tmax2, maxDelayInt, DTint;
	Int64 dtInt = round(baseF*dt);
	...
	if ( histlen<1 )
	{
		printf("If you discover how to make a histogram using %ld bins, let me know. Bye.", histlen);
		return -4;
	}
...
Int64 determineCoincidenceHistogram(Int64* tags, Int8* channels, Int64 valid, Int64 ch1, Int64 ch2, double dt, double T1, double T2, Int64* hist, Int64 histlen)
{
	...
	Int64 T1int,
	      T2int, maxt1, mmod,
	      Tmin1, Tmin2, Tmax1, Tmax2, maxDelayInt, dtInt = round(dt*baseF), DTint;
	Int64 i, j, t1, t2, m;
	...
	if ( valid < 2 )
	{
		printf("ERROR: if valid is not at least 2, you'll never get coincidences...\n");
		return -3;
	}

	if ( histlen<1 )
	{
		printf("ERROR: If you discover how to make a histogram using %ld bins, let me know. Bye.", histlen);
		return -4;
	}
