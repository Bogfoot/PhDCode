...
if __name__ == "__main__":
    ...
    closeup = frame[
        xmax - boxsize // 2 : xmax + boxsize // 2,
        ymax - boxsize // 2 : ymax + boxsize // 2,
        2,
    ]

    xstd = (
        frame[:, ymax - averagingsize // 2 : ymax + averagingsize // 2, 2]
        .mean(axis=1)
        .std()
    )
    ystd = (
        frame[xmax - averagingsize // 2 : xmax + averagingsize // 2, :, 2]
        .mean(axis=0)
        .std()
    )
    ...
    ax[2].text(x=0.50, y=0.8, s=r"$\sigma_x = {:.2f}$".format(xstd), fontsize=fontsize)
    ax[2].text(x=0.50, y=0.20, s=r"$\sigma_y = {:.2f}$".format(ystd), fontsize=fontsize)
