 ...
 *
 */
int rp_read_calib_params(rp_ext_calib_params_t *calib_params)
{
    FILE   *fp;
    size_t  size;
    ...
    size=fread(calib_params, sizeof(char), (int)sizeof(rp_ext_calib_params_t), fp);
    if(size != sizeof(rp_ext_calib_params_t)) {
        fclose(fp);
        fprintf(stderr, "rp_read_calib_params(): fread() failed, "
                "returned bytes: %d (should be :%d)\n", (int)size, 
                (int)sizeof(rp_ext_calib_params_t));
        return -1;
    }
    fclose(fp);
...
int rp_default_calib_params(rp_ext_calib_params_t *calib_params)
{
